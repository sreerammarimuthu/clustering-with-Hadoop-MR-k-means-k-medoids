...
public class KMeans_AMI {
    ...
    public static void main(String[] args) throws Exception {
        ...
        outputPath = "C://Users//user//Downloads//Output_Task_2c_k77";
        ...
        List<KMeans_AMI.ClusterCenter> seedPoints = new ArrayList<>();
        try (BufferedReader br = new BufferedReader(new FileReader(kSeedData))) {
            ...
            while (j < k && (line = br.readLine()) != null) { // Read the first k lines
                ...
                double x = Double.parseDouble(parts[0]);
                double y = Double.parseDouble(parts[1]);
                seedPoints.add(new KMeans_AMI.ClusterCenter(j + 1, new KMeans_AMI.Point(x, y)));
                j++;
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        List<KMeans_AMI.ClusterCenter> selectedCentroids = getRandomCentroids(seedPoints, k);
        ...
        job.setJarByClass(KMeans_AMI.class);
        job.setMapperClass(KMeans_AMI.KMeansMapper.class);
        job.setReducerClass(KMeans_AMI.KMeansReducer.class);
